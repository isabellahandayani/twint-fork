# Taken from https://github.com/trevorhobenshield/twitter-api-client
    ...
    try:
        ...
        for s in info.get("subtasks", []):
            if s.get("enter_text", {}).get("keyboard_type") == "email":
                print(f"[warning] {' '.join(find_key(s, 'text'))}")
                session.cookies.set(
                    "confirm_email", "true"
...
def init_guest_token(session: Client) -> Client:
    return update_token(
        session, "guest_token", "https://api.twitter.com/1.1/guest/activate.json"
    )
                        ...
                        "setting_responses": [
                            {
                                ...
                                "response_data": {
                                    "text_data": {
                                        "result": session.cookies.get("username")
                                    }
                                },
                            }
                        ],
            ...
            "subtask_inputs": [
                {
                    ...
                    "enter_password": {
                        "password": session.cookies.get("password"),
                        "link": "next_link",
                    },
                }
            ],
...
def login(email: str, username: str, password: str) -> Client:
    ...
    if session.cookies.get("flow_errors") == "true":
        print(f"[error] {username} login failed")
    else:
        print(f"[success] {username} login success")
    return session
