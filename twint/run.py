...
class Twint:
    def __init__(self, config):
        ...
        self.session = login(
            config.Account_email, config.Account_username, config.Account_password
        )
    ...
    async def Feed(self):
        ...
        while True:
            ...
            try:
                response = await get.RequestUrl(self.config, self.init, self.session)
    ...
    async def follow(self):
        ...
        if self.config.User_full:
            ...
            if self.conn_postgres:
                self.count += await get.Multi(
                    self.feed, self.config, self.conn_postgres
                )
            else:
        ...
        if self.config.Username is not None and self.config.User_id is None:
            ...
            if self.conn_postgres:
                self.config.User_id = await get.User(
                    self.config.Username, self.config, self.conn_postgres, True
                )
            else:
                self.config.User_id = await get.User(
        ...
        try:
            ...
            if self.config.DatabasePostgres:
                await get.User(
                    ...
                    self.config,
                    postgres.ConnPostgres(self.config.DatabasePostgres),
                )
            else:
                await get.User(
