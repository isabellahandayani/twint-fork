...
class Scrapper:
    ...
    def search(self, keywords, keywords_exact='', keywords_or='', keywords_stop='', hashtags='', min_replies=0, min_retweets=0, min_likes=0, since='', until='', limit=100):
        
        keywords_exact = f'"{keywords_exact}"' if keywords_exact != '' else '\b'
        keywords_stop = ' '.join(['-' + keyword for keyword in keywords_stop.split(' ')]) if len(keywords_stop) > 0 else '\b'
        keywords_or = '(' + ' OR '.join(keywords_or.split(' ')) + ')' if len(keywords_or) > 0 else '\b'
        hashtags = '(' + ' OR '.join(hashtags.split(' ')) + ')' if len(hashtags) > 0 else '\b'
